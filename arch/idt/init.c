...
static uint64_t gdt64_runtime[5];
...
static inline uint64_t build_code_segment_descriptor(void) {
    return (1ULL << 43) | (1ULL << 44) | (1ULL << 47) | (1ULL << 53);
}
...
void gdt_tss_init() {
    ...
    gdt64_runtime[0] = 0;
    gdt64_runtime[1] = build_code_segment_descriptor();
    gdt64_runtime[2] = 0;

    fill_tss_descriptor(&kernel_tss_desc, &kernel_tss);
    gdt64_runtime[3] = *(uint64_t*)&kernel_tss_desc;
    gdt64_runtime[4] = ((uint64_t)kernel_tss_desc.base_upper << 32) | kernel_tss_desc.reserved;
...
void idt_attach_handler_ist(int n, void* handler, uint8_t ist_index) {
    ...
    idt[n].selector = GDT_KERNEL_CODE_SELECTOR;
