...
$(kernel_object_files): build/kernel/%.o : src/implementations/kernel/%.c
	mkdir -p $(dir $@) && \
	x86_64-elf-gcc -c -I src/interface -ffreestanding $(patsubst build/kernel/%.o, src/implementations/kernel/%.c, $@) -o $@

$(x86_64_c_object_files): build/x86_64/%.o : src/implementations/x86_64/%.c
	mkdir -p $(dir $@) && \
	x86_64-elf-gcc -c -I src/interface -ffreestanding $(patsubst build/x86_64/%.o, src/implementations/x86_64/%.c, $@) -o $@
